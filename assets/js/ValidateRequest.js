...
function displayValidationErrors() {
    ...
    for (var i = 0, len = finalResponse.length; i < len; i++) {
        temp = '<tr><td>' + finalResponse[i].lineNumber + '</td>';
        temp += '<td>' + finalResponse[i].command + '</td>';
        temp += '<td>' + finalResponse[i].desc + '</td>';
        temp += '<td>' + finalResponse[i].suggestion + '</td></tr>';
        $('table .validation-body').append(temp);
...
function displayTrivyResults(data) {
    ...
    let responseList = data['responseList'];
    console.log(responseList)
    if (data['responseList'][0] && data['responseList'][0]['Results'] && data['responseList'][0]['Results'][0]['Misconfigurations']) {
        let MisconfSummary_result =
            data['responseList'][0]['Results'][0];

        Misconfigurations = JSON.parse(
            JSON.stringify(MisconfSummary_result['Misconfigurations'])
        );
    ...
    for (var i = 0, len = Misconfigurations.length; i < len; i++) {
        temp = '<tr><td>' + Misconfigurations[i].Type + '</td>';
        temp += '<td>' + Misconfigurations[i].ID + '</td>';
        temp += '<td>' + Misconfigurations[i].Title + '</td>';
        temp += '<td>' + Misconfigurations[i].Description + '</td></tr>';
        $('table .trivy-body').append(temp);
    }
    document.getElementById("trivy-results").style.display = "flex";
...
function validateEmailAndContent() {
    ...
    if (document.querySelector("#email").checkValidity() == false)
        document.getElementById("email-validation").innerHTML = "Please enter the valid Email ID";
    else
        document.getElementById("email-validation").innerHTML = "";

    if (document.querySelector("#dockercontent").checkValidity() == false)
        document.getElementById("content-validation").innerHTML = "Please enter the Dockerfile content";
    else
        document.getElementById("content-validation").innerHTML = "";

    if (document.querySelector("#email").checkValidity() == false || document.querySelector("#dockercontent").checkValidity() == false)
        return false;
    else
        return true;
}
...
function validateDockerfile() {
    ...
    let validationPassed = validateEmailAndContent();
    if (validationPassed) {
        ...
        var contents = requestValue;

        var commands = parse(contents, options);
        //console.log(commands)
        commands.every(function (cmd) {

            if (cmd.name === "FROM" || cmd.name === "ARG") {
                if (!(requestValue.search(/\FROM\b/) >= 0))
            ...
            if (words[0] === "EXPOSE") {
                if (words[1] && words[2]) {
                    ...
                    if (words[1].includes("$") || words[1].includes("{") || words[1].includes("}")) {
                        ...
                        setFailureStatus(lineCount, "EXPOSE", "The argument should be a port number", "EXPOSE 8080 (OR) EXPOSE 80/tcp")

                }
                else
                    setFailureStatus(lineCount, "EXPOSE", "EXPOSE should have an argument", "EXPOSE 80/tcp")
            }
        ...
        if (requestValidationPassed == true && requestValidationFailed == false) {
            ...
            callAPI(requestValue).then(data => { console.log(data); displayTrivyResults(data); });
            requestValidationPassed == false;
        }
